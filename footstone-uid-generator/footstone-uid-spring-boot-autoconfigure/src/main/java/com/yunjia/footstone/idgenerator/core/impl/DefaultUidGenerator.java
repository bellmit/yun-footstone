 ...
 * <li>epochStr: Epoch date string format 'yyyy-MM-dd'. Default as '2021-12-01'<p>
 *
 * <b>Note that:</b> The total bits must be 64 -1
 * </p>
 *
 * @author sunkaiyun
 * @date 2021/11/28 1:59 PM
 */
@Slf4j
public class DefaultUidGenerator implements UidGenerator, InitializingBean {
    ...
    public DefaultUidGenerator(BaseUidProperties baseUidProperties, WorkerIdAssigner workerIdAssigner) {
        if(null == baseUidProperties) {
            baseUidProperties = new BaseUidProperties();
        }
        this.timeBits = baseUidProperties.getTimeBits();
        this.workerBits = baseUidProperties.getWorkerBits();
        this.seqBits = baseUidProperties.getSeqBits();
        this.epochStr = baseUidProperties.getEpochStr();
        if(null == workerIdAssigner) {
            this.workerIdAssigner = new DisposableWorkerIdAssigner();
        } else {
