 ...
 * 配置文件的构建工厂类
 * </p>
 * 用于手动读取配置文件初始化 AuthConfig 对象，只有在非IOC环境下才会用到此类
 *
 * @author sunkaiyun
 * @date 2021/12/24 6:08 PM
 */
public class AuthConfigFactory {
     ...
     * 配置文件地址
     */
    public static String configPath = "auth.properties";

    /**
     * 根据configPath路径获取配置信息
     *
     * @return 一个AuthConfig对象
     */
    public static AuthConfig createConfig() {
        Map<String, String> map = readPropToMap(configPath);
        return (AuthConfig) initPropByMap(map, new AuthConfig());
    }
    ...
    private static Map<String, String> readPropToMap(String propertiesPath) {
        ...
        try {
            InputStream is = AuthConfigFactory.class.getClassLoader().getResourceAsStream(propertiesPath);
            if(is == null) {
                return null;
            }
