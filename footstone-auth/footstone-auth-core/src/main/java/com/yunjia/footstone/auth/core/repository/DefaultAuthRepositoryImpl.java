 ...
 */
package com.yunjia.footstone.auth.core.repository;
...
/**
 * <p>
 * Auth持久层接口 [默认实现类, 基于内存Map]
 * </p>
 *
 * @author sunkaiyun
 * @date 2021/12/27 10:41 AM
 */
@Slf4j
public class DefaultAuthRepositoryImpl implements AuthRepository {
     ...
     * 构造函数
     */
    public DefaultAuthRepositoryImpl() {
        initRefreshThread();
    }
    ...
    public void set(String key, String value, long timeout) {
        ...
        expireMap.put(key, (timeout == AuthRepository.NEVER_EXPIRE) ? (AuthRepository.NEVER_EXPIRE)
                : (System.currentTimeMillis() + timeout * 1000));
    }

    @Override
    public void update(String key, String value) {
        if(getKeyTimeout(key) == AuthRepository.NOT_VALUE_EXPIRE) {
            return;
        }
        dataMap.put(key, value);
    }
    ...
    void clearKeyByTimeout(String key) {
        ...
        if(expirationTime != null && expirationTime != AuthRepository.NEVER_EXPIRE
    ...
    long getKeyTimeout(String key) {
        ...
        // 如果被标注为永不过期
        if(expire == AuthRepository.NEVER_EXPIRE) {
